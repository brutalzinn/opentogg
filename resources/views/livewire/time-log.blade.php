        ...
        @else
            ...
            <div class="bg-surface-raised p-3 sm:p-4 mb-2 rounded-xl flex items-center justify-between gap-3">
                        ...
                        @if($entry->tags->isNotEmpty())
                            <span class="flex flex-wrap gap-1">
                                @foreach($entry->tags as $tag)
                                    <span class="text-xs bg-accent/20 text-accent px-1.5 py-0.5 rounded">#{{ $tag->name }}</span>
                                @endforeach
                            </span>
                        @endif
                ...
                <div class="flex shrink-0">
                    <button
                        wire:click="$dispatch('continue-entry', { entryId: {{ $entry->id }} })"
                        class="text-accent hover:text-accent-hover p-3 [touch-action:manipulation]"
                        aria-label="{{ __('app.entry_continue') }}"
                        title="{{ __('app.entry_continue') }}"
                    >
                    ...
                    <button
                        ...
                        class="text-danger hover:text-danger/80 p-3 [touch-action:manipulation]"
